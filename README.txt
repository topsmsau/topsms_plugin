=== TopSMS ===
Contributors: euxdigital
Tags: woocommerce, sms, notifications, order status, analytics
...
Enhance your WooCommerce store with automated SMS notifications based on order status changes
...
* Personalize messages with dynamic variables (customer name, order ID, tracking number, etc.)
* Schedule delayed notifications for specific order statuses
* Supports multiple languages for global stores
...
Yes, TopSMS is fully GDPR compliant. The plugin includes consent management tools, data retention policies, and the ability for customers to opt-out of SMS notifications at any time.


= Can I send SMS notifications for custom order statuses? =
No! At present this facility is not supported
